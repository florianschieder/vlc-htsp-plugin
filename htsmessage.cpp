...
bool HtsMap::contains(const std::string &name) { return data.count(name) > 0; }
...
void HtsMap::setData(const std::string &name,
                     std::shared_ptr<HtsData> newData) {
...
uint32_t HtsList::count() { return data.size(); }
...
HtsBin::HtsBin(const HtsBin &other) { other.getBin(&data_length, &data_buf); }
...
uint32_t HtsMap::calcSize() { return pCalcSize() + 6 + getName().length(); }
...
uint32_t HtsList::calcSize() { return pCalcSize() + 6 + getName().length(); }
...
uint32_t HtsInt::calcSize() { return pCalcSize() + getName().length() + 6; }
...
uint32_t HtsStr::calcSize() { return data.length() + getName().length() + 6; }
...
uint32_t HtsBin::calcSize() { return data_length + getName().length() + 6; }
