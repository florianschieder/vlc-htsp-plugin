...
struct hts_stream {
    hts_stream() : index(0), es(0), lastDts(0), lastPts(0), ignoreTime(false) {}
...
struct demux_sys_t : public sys_common_t {
    demux_sys_t()
        : lastPcr(0), ptsDelay(300000), currentPcr(0), tsOffset(0), tsStart(0),
          tsEnd(0), timeshiftPeriod(0), streamCount(0), stream(0),
          audioOnly(false), host(""), port(0), username(""), password(""),
          channelId(0), hadIFrame(false), drops(0), epg(0),
          thread(vlc_thread_t{.handle = 0}), requestSpeed(INT_MIN),
          requestSeek(-1), doDisable(false) {
...
bool ConnectHTSP(demux_t *demux) {
    ...
    msg_Info(demux, "Connected to HTSP Server %s, version %s, protocol %d",
             sys->serverName.c_str(), sys->serverVersion.c_str(),
             sys->protoVersion);
    if (sys->protoVersion < HTSP_PROTO_VERSION) {
        msg_Warn(demux,
                 ...
                 sys->protoVersion, HTSP_PROTO_VERSION);
    ...
    if (sys->password != "" && chall) {
        msg_Info(demux, "Authenticating as '%s' with a password",
                 sys->username.c_str());

        HTSSHA1 *shactx = (HTSSHA1 *)malloc(hts_sha1_size);
        uint8_t d[20];
        hts_sha1_init(shactx);
        hts_sha1_update(shactx, (const uint8_t *)(sys->password.c_str()),
                        sys->password.length());
        ...
        msg_Info(demux, "Authenticating as '%s' without a password",
                 sys->username.c_str());
    }
...
int ControlHTSP(demux_t *demux, int i_query, va_list args) {
    ...
    switch (i_query) {
        ...
        case DEMUX_GET_POSITION:
            ...
            *va_arg(args, double *) =
                (sys->currentPcr - sys->tsStart) / totalTime;
            return VLC_SUCCESS;
        ...
        case DEMUX_GET_PTS_DELAY:
            *va_arg(args, int64_t *) =
                INT64_C(1000) * var_InheritInteger(demux, "network-caching");
            return VLC_SUCCESS;
        case DEMUX_GET_TIME:
            if (sys->currentPcr == 0) {
                return VLC_EGENERIC;
...
void *RunHTSP(void *obj) {
    ...
    for (;;) {
        ...
        if (sys->doDisable) {
            ...
            for (auto it = sys->disables.begin(); it != sys->disables.end();
                 ++it) {
...
bool ParseSubscriptionStart(demux_t *demux, HtsMessage &msg) {
    ...
    if (msg.getRoot()->contains("sourceinfo") && sys->epg != 0) {
        ...
        es_out_Control(demux->out, ES_OUT_SET_GROUP_META, (int)sys->channelId,
                       meta);
    ...
    for (uint32_t jj = 0; jj < streams->count(); jj++) {
        ...
        msg_Dbg(demux, "Found elementary stream id %d, type %s", index,
                type.c_str());
    }
...
bool ParseSubscriptionStop(demux_t *demux, HtsMessage &msg) {
    msg_Info(demux, "HTS Subscription Stop: subscriptionId: %d, status: %s",
             msg.getRoot()->getU32("subscriptionId"),
             msg.getRoot()->getStr("status").c_str());
    return false;
}

bool ParseSubscriptionStatus(demux_t *demux, HtsMessage &msg) {
    msg_Dbg(demux, "HTS Subscription Status: subscriptionId: %d, status: %s",
            msg.getRoot()->getU32("subscriptionId"),
            msg.getRoot()->getStr("status").c_str());
    return true;
}
...
bool ParseQueueStatus(demux_t *demux, HtsMessage &msg) {
    ...
    uint32_t drops = msg.getRoot()->getU32("Bdrops") +
                     msg.getRoot()->getU32("Pdrops") +
                     msg.getRoot()->getU32("Idrops");
    if (drops > sys->drops) {

        msg_Warn(demux, "Can't keep up! HTS dropped %d frames!",
                 drops - sys->drops);
        msg_Warn(
            demux,
            "HTS Queue Status: subscriptionId: %d, Packets: %d, Bytes: %d, "
            "Delay: %lld, Bdrops: %d, Pdrops: %d, Idrops: %d",
            msg.getRoot()->getU32("subscriptionId"),
            msg.getRoot()->getU32("packets"), msg.getRoot()->getU32("bytes"),
            (long long int)msg.getRoot()->getS64("delay"),
            msg.getRoot()->getU32("Bdrops"), msg.getRoot()->getU32("Pdrops"),
            msg.getRoot()->getU32("Idrops"));
...
bool ParseMuxPacket(demux_t *demux, HtsMessage &msg) {
    ...
    if (index == 0) {
        free(bin);
        msg_Err(demux, "Invalid stream index detected: %d with %d streams",
                index, sys->streamCount);
        return false;
    }
    ...
    mtime_t pcr = 0;
    for (uint32_t i = 0; i < sys->streamCount; i++) {
        if (sys->stream[i].lastDts > 0 &&
            (sys->stream[i].lastDts < pcr || pcr == 0)) {
...
bool ParseSubscriptionSkip(demux_t *demux, HtsMessage &msg) {
    ...
    if (msg.getRoot()->contains("error") || msg.getRoot()->contains("size") ||
        !msg.getRoot()->contains("time")) {
        return true;
    }
    ...
    msg_Info(demux, "SubscriptionSkip: newTime: %lld, base: %s",
             (long long int)newTime,
             (msg.getRoot()->getU32("absolute")) ? "abs" : "rel");
...
int DemuxHTSP(demux_t *demux) {
    ...
    if (method == "muxpkt") {
        ...
        msg_Warn(demux, "Ignoring packet of unknown method \"%s\"",
                 method.c_str());
    }
